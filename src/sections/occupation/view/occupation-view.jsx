...
export default function OccupationView() {
  ...
  useEffect(() => {
    dispatch(actGetOccupationsAsync({ page: page + 1, pageSize: rowsPerPage }));
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = occupations.map((n) => n.name);
  ...
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
    dispatch(actGetOccupationsAsync({ page: newPage + 1, pageSize: rowsPerPage })); // Cập nhật trang và gọi API
  };
  const handleChangeRowsPerPage = (event) => {
    const newRowsPerPage = parseInt(event.target.value, 10);
    setRowsPerPage(newRowsPerPage);
    setPage(0); // Reset về trang đầu tiên khi thay đổi số lượng
    dispatch(actGetOccupationsAsync({ page: 1, pageSize: newRowsPerPage })); // Gọi API với `pageSize` mới
  };
  ...
  const handleFilterByName = async (event) => {
    ...
    if (filterValue.trim()) {
      ...
      dispatch(actGetOccupationsAsync({ page: 1, pageSize: rowsPerPage }));
    }
  };
