...
export default function EntryLevelEducationView() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = entryLevelEducations.map((n) => n.name);
  ...
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
    dispatch(actGetEntryLevelEducationsAsync({ page: newPage + 1, pageSize: rowsPerPage })); // Cập nhật trang và gọi API
  };
  const handleChangeRowsPerPage = (event) => {
    const newRowsPerPage = parseInt(event.target.value, 10);
    setRowsPerPage(newRowsPerPage);
    setPage(0); // Reset về trang đầu tiên khi thay đổi số lượng
    dispatch(actGetEntryLevelEducationsAsync({ page: 1, pageSize: newRowsPerPage })); // Gọi API với `pageSize` mới
  };
